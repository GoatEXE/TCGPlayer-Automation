import type { InvoiceData, RenderableDate } from './types.js';
...
export function renderInvoiceHtml(data: InvoiceData): string {
  ...
  const totalCents =
    data.totalCents ?? subtotalCents + shippingCents + taxCents;
  const sellerName = data.seller.sellerName.trim();
  const paymentMethod = data.paymentMethod?.trim() || 'TCGPlayer marketplace';
    ...
    ? `
      <section class="panel">
        ...
        <dl>
          ...
          ${renderOptionalDefinition('Email', data.seller.contactEmail)}
          ${renderOptionalDefinition('Address', data.seller.address, true)}
          ${renderOptionalDefinition('Phone', data.seller.phone)}
        </dl>
      </section>
    `
    : '';
    ...
    .map(
      (item) => `
        <tr>
          ...
          <td class="number">${formatCurrency(item.unitPriceCents)}</td>
          <td class="number">${formatCurrency(item.lineTotalCents)}</td>
        </tr>
      `,
    )
    .join('');

  const contactLine = data.seller.contactEmail?.trim()
    ? `<p>Questions? Contact ${escapeHtml(data.seller.contactEmail)}.</p>`
    : '';
...
<html lang="en">
  <head>
    ...
    <title>Invoice - Order #${escapeHtml(data.orderId)}</title>
      ...
      <header class="header">
        ...
        <section class="panel">
          ...
          <dl>
            ...
            <dt>Payment</dt>
            <dd>${escapeHtml(paymentMethod)}</dd>
            <dt>Buyer</dt>
      ...
      <section class="summary">
        <table aria-label="Invoice totals">
          <tbody>
            <tr>
              <th>Subtotal</th>
              <td class="number">${formatCurrency(subtotalCents)}</td>
            </tr>
            <tr>
              <th>Shipping</th>
              <td class="number">${formatCurrency(shippingCents)}</td>
            </tr>
            <tr>
              <th>Tax</th>
              <td class="number">${formatCurrency(taxCents)}</td>
            </tr>
            <tr>
              <th>Total</th>
              <td class="number">${formatCurrency(totalCents)}</td>
            </tr>
          </tbody>
        </table>
      </section>
