...
export type SellabilityAction = 'sell' | 'keep' | 'excluded';
...
export interface ExistingCollectionCounts {
  normalQty: number;
  foilQty: number;
}

export interface ScanSessionItemInput {
  catalogCardId: number;
  quantity: number;
  finish?: string | null;
  condition?: string | null;
...
export interface ScanPreviewAllocation {
  action: SellabilityAction;
  ...
  quantity: number;
  keepTarget: number | null;
  existingNormalQty: number;
  existingFoilQty: number;
  scannedNormalQty: number;
  scannedFoilQty: number;
  excluded: boolean;
  excludedReason: string | null;
  needsClassification: boolean;
  reasons: string[];
...
export interface ScanPreviewResult {
  ...
  summary: {
    ...
    needsClassificationQuantity: number;
  };
  items: ScanPreviewAllocation[];
  groups: {
    sell: ScanPreviewAllocation[];
    keep: ScanPreviewAllocation[];
    excluded: ScanPreviewAllocation[];
  };
}
...
function makeAllocation(params: {
  ...
  finishKind: FinishKind;
  quantity: number;
  scanSample: ScanSessionItemInput;
  counts: ExistingCollectionCounts & { scannedNormalQty: number; scannedFoilQty: number };
  ...
  return {
    ...
    ...cardIdentity(params.card),
    kind: params.classified.kind,
    kindSource: params.classified.kindSource,
    cardKindSource: params.classified.cardKindSource,
    finish: params.scanSample.finish?.trim() || DEFAULT_FINISH,
    finishKind: params.finishKind,
    condition: params.scanSample.condition?.trim() || DEFAULT_CONDITION,
    language: params.scanSample.language?.trim().toUpperCase() || DEFAULT_LANGUAGE,
    quantity: params.quantity,
    keepTarget: params.allocation.keepTarget,
    existingNormalQty: params.counts.normalQty,
    existingFoilQty: params.counts.foilQty,
    scannedNormalQty: params.counts.scannedNormalQty,
    scannedFoilQty: params.counts.scannedFoilQty,
    excluded: params.allocation.excluded,
    excludedReason: params.allocation.excludedReason,
    needsClassification:
      params.allocation.needsClassification || params.classified.needsClassification,
    reasons: params.allocation.reasons,
    reasonCodes: params.allocation.reasonCodes,
    primaryReasonCode:
      params.action === 'sell' &&
      params.finishKind === 'foil' &&
      params.allocation.keepTargetSatisfiedByNormal
        ? 'foil_preference'
        : params.allocation.primaryReasonCode,
    opportunityType:
      params.action === 'sell'
        ? params.finishKind === 'foil' &&
          params.allocation.keepTargetSatisfiedByNormal
...
export function computeScanSessionPreview(params: {
  scannedItems: ScanSessionItemInput[];
  catalogCards: SellabilityCatalogCardInput[];
  existingCountsByCatalogCardId?: Map<number, ExistingCollectionCounts>;
}): ScanPreviewResult {
  const catalogById = new Map(
    params.catalogCards.map((card) => [card.catalogCardId, card]),
  );
  const scannedByCard = new Map<
    number,
    {
      samples: Map<FinishKind, ScanSessionItemInput>;
      normalQty: number;
      foilQty: number;
    }
  >();

  for (const item of params.scannedItems) {
    const group = scannedByCard.get(item.catalogCardId) ?? {
      samples: new Map<FinishKind, ScanSessionItemInput>(),
      normalQty: 0,
      foilQty: 0,
    };
    const finishKind = normalizeFinishKind(item.finish);
    if (!group.samples.has(finishKind)) {
      group.samples.set(finishKind, item);
    }
    if (finishKind === 'foil') {
      group.foilQty += item.quantity;
    } else {
      group.normalQty += item.quantity;
    }
    scannedByCard.set(item.catalogCardId, group);
  }

  const allocations: ScanPreviewAllocation[] = [];

  for (const [catalogCardId, scanned] of scannedByCard.entries()) {
    const card = catalogById.get(catalogCardId);
    if (!card) {
      continue;
    }

    const existing = params.existingCountsByCatalogCardId?.get(catalogCardId) ?? {
      normalQty: 0,
      foilQty: 0,
    };
    const classified = classifyCatalogCard(card);
    const totalNormalQty = existing.normalQty + scanned.normalQty;
    const totalFoilQty = existing.foilQty + scanned.foilQty;
    const allocation = computeAllocations(
      classified.kind,
      totalNormalQty,
      totalFoilQty,
    );
    const scannedKeepNormalQty = Math.max(
      Math.min(scanned.normalQty, allocation.keepNormalQty - existing.normalQty),
      0,
    );
    const scannedKeepFoilQty = Math.max(
      Math.min(scanned.foilQty, allocation.keepFoilQty - existing.foilQty),
      0,
    );
    const scannedSellNormalQty = Math.max(scanned.normalQty - scannedKeepNormalQty, 0);
    const scannedSellFoilQty = Math.max(scanned.foilQty - scannedKeepFoilQty, 0);
    const counts = {
      ...existing,
      scannedNormalQty: scanned.normalQty,
      scannedFoilQty: scanned.foilQty,
    };

    const normalSample = scanned.samples.get('normal') ?? {
      catalogCardId,
      quantity: scanned.normalQty,
      finish: DEFAULT_FINISH,
    };
    const foilSample = scanned.samples.get('foil') ?? {
      catalogCardId,
      quantity: scanned.foilQty,
      finish: 'Foil',
    };
    ...
    const pushSplit = (
      ...
      keepQty: number,
      sellQty: number,
      sample: ScanSessionItemInput,
    ) => {
      const keepAction: SellabilityAction = allocation.excluded
        ? 'excluded'
        : 'keep';
      if (sellQty > 0) {
        allocations.push(
    ...
    pushSplit('foil', scannedKeepFoilQty, scannedSellFoilQty, foilSample);
    pushSplit('normal', scannedKeepNormalQty, scannedSellNormalQty, normalSample);
  }

  const actionRank: Record<SellabilityAction, number> = {
    sell: 0,
    keep: 1,
    excluded: 2,
  };
  const sortedItems = allocations.sort((left, right) => {
  ...
  const sell = sortedItems.filter((item) => item.action === 'sell');
  const keep = sortedItems.filter((item) => item.action === 'keep');
  const excluded = sortedItems.filter((item) => item.action === 'excluded');
  ...
  return {
    ...
    summary: {
      sellQuantity: sell.reduce((sum, item) => sum + item.quantity, 0),
      keepQuantity: keep.reduce((sum, item) => sum + item.quantity, 0),
      excludedQuantity: excluded.reduce((sum, item) => sum + item.quantity, 0),
      needsClassificationQuantity: sortedItems
