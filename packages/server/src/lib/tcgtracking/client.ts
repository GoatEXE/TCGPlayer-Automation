...
export class TCGTrackingClient {
  ...
  constructor(baseUrl?: string) {
    this.baseUrl =
      baseUrl || process.env.TCGTRACKING_BASE_URL || DEFAULT_BASE_URL;
  }
  ...
  async getSets(): Promise<TCGTrackingSet[]> {
    ...
    try {
      ...
      if (!data.sets || !Array.isArray(data.sets)) {
        console.warn(
          'TCGTracking getSets returned unexpected format (missing sets array)',
        );
        return [];
      }
  ...
  async getPricing(setId: number): Promise<TCGTrackingPriceResponse | null> {
    ...
    try {
      ...
      if (!data || typeof data !== 'object' || !data.prices) {
        console.warn(
          'TCGTracking getPricing returned unexpected format (missing prices object)',
        );
        return null;
      }
      ...
      console.error(
        `Error fetching pricing for set ${setId} from TCGTracking:`,
        error,
      );
      return null;
    }
