...
function mockInsertWithReturning() {
  vi.mocked(db.insert).mockImplementation(
    () =>
      ({
        values: vi.fn((value) => ({
          onConflictDoNothing: vi.fn().mockResolvedValue(undefined),
          returning: vi.fn().mockResolvedValue([{ id: 999, ...value }]),
        })),
      }) as any,
  );
}
...
describe('collection routes', () => {
  ...
  it('bulk inserts scanner items with default ownership fields and accepts web string ids', async () => {
    vi.mocked(db.select)
      .mockReturnValueOnce(selectRows([{ id: 1, name: 'Default' }]) as any)
    ...
    const response = await app.inject({
      method: 'POST',
      url: '/api/collections/1/items/bulk',
      payload: { items: [{ catalogCardId: '100' }] },
    });
    ...
    expect(JSON.parse(response.body)).toMatchObject({
      ...
      updated: 0,
      errors: [],
      items: [{ quantity: 1, condition: 'Near Mint', finish: 'Normal' }],
    });
    expect(db.insert).toHaveBeenCalledWith(collectionItems);
    expect(db.insert).not.toHaveBeenCalledWith(cards);
  });

  it('previews scanner session split recommendations using existing owned counts', async () => {
    vi.mocked(db.insert).mockReturnValue({
      values: vi.fn().mockReturnValue({
        onConflictDoNothing: vi.fn().mockResolvedValue(undefined),
      }),
    ...
    vi.mocked(db.select)
        ...
        selectRows([
          {
            catalogCardId: 100,
            tcgProductId: 700100,
            productName: 'Extra Unit',
            title: null,
            collectorNumber: '100/200',
            normalizedNumber: '100/200',
            rarity: null,
            photoUrl: null,
            cardKind: 'normal',
            set: { id: 10, setCode: 'UNL', name: 'Origins' },
          },
          {
            catalogCardId: 101,
            tcgProductId: 700101,
            productName: 'Hero Legend',
            title: null,
            collectorNumber: '101/200',
            normalizedNumber: '101/200',
            rarity: null,
            photoUrl: null,
            cardKind: 'legend',
            set: { id: 10, setCode: 'UNL', name: 'Origins' },
      ...
      .mockReturnValueOnce(
        selectRows([
          { catalogCardId: 100, quantity: 2, finish: 'Normal' },
          { catalogCardId: 101, quantity: 1, finish: 'Normal' },
        ]) as any,
      );

    const response = await app.inject({
      method: 'POST',
      url: '/api/collections/scan-preview',
      payload: {
        items: [
          { catalogCardId: 100, quantity: 1, finish: 'Normal' },
          { catalogCardId: 100, quantity: 1, finish: 'Foil' },
          { catalogCardId: 101, quantity: 1, finish: 'Foil' },
          { catalogCardId: 102, quantity: 1, finish: 'Normal' },
        ],
      },
    });
    ...
    expect(body.groups.sell).toEqual(
    ...
    expect(body.groups.excluded).toEqual([
      expect.objectContaining({ catalogCardId: 102, kind: 'rune' }),
    ]);
    expect(db.insert).not.toHaveBeenCalledWith(cards);
  ...
  it('split commit writes sell rows to To Be Sold and keep rows to Default without touching cards inventory', async () => {
    mockInsertWithReturning();
    vi.mocked(db.select)
      .mockReturnValueOnce(selectRows([{ id: 1, name: 'Default', purpose: 'owned' }]) as any)
      .mockReturnValueOnce(selectRows([{ id: 2, name: 'To Be Sold', purpose: 'to_be_sold' }]) as any)
      .mockReturnValueOnce(selectRows([]) as any)
    ...
    const response = await app.inject({
      method: 'POST',
      url: '/api/collections/split-scan',
      payload: {
        allocations: [
          {
            action: 'sell',
    ...
    expect(response.statusCode).toBe(200);
    expect(JSON.parse(response.body)).toMatchObject({
      summary: { sellQuantity: 1, keepQuantity: 2 },
    });
    expect(db.transaction).toHaveBeenCalled();
    expect(db.insert).toHaveBeenCalledWith(collectionItems);
    expect(db.insert).not.toHaveBeenCalledWith(cards);
  ...
  it('bulk increments existing collection items by merge key without touching cards inventory', async () => {
    vi.mocked(db.select)
      .mockReturnValueOnce(selectRows([{ id: 1, name: 'Default' }]) as any)
      .mockReturnValueOnce(
        selectRows([
    ...
    const response = await app.inject({
      method: 'POST',
      url: '/api/collections/1/items/bulk',
      payload: { items: [{ catalogCardId: 100, quantity: 3 }] },
    });
